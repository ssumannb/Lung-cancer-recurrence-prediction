...
%%% loop index
loop_primary = ["peri";"comb"];%"intra";"peri";"comb"];%;"comb"];
loop_peri = ["3mm"; "12mm"];
loop_comb = ["6mm"; "9mm"];
%%% i_ref_B version
...
function foldLoop(p_regionType, p_clf, varargin)
    ...
    %%% 변수 공유 안돼서 다시 선언
    date_str = "210506";
    path_dirbase_csv = 'E:/research/Prediction/NSCLC/Codes/Clinical_Radiomic_combine/';
    path_save_base = strcat(path_dirbase_csv,'output(',date_str,')/',p_regionType,'/');
    loop_fold = ["A"; "B"; "C"; "D"; "E"];
    %%%
    ...
    for idx_fold = 1:size(loop_fold)
       ...
       %% train/test data read
        ...
        if p_regionType == 'cln'
            ...
            csv_path_train = strcat(path_dirbase_csv,'fold(',date_str,')/', p_regionType,'_testfold(',fold,')_train.csv');  % csv that clinical feaeture saved (train)
            csv_path_test = strcat(path_dirbase_csv,'fold(',date_str,')/', p_regionType,'_testfold(',fold,')_test.csv');    % csv that clinical feaeture saved (test)
            ...
            % A:1 B:2 C:3 D:4 E:5
            csv_path_train = strcat(path_dirbase_csv,'fold(',date_str,')/', 'trainfold_gt_',fold,'.csv');   % csv that ground truth saved (train)
            csv_path_test = strcat(path_dirbase_csv,'fold(',date_str,')/', 'testfold_gt_',fold,'.csv');     % csv that ground truth saved (test)
            ...
            mat_path = strcat(path_dirbase_csv, 'fold(', date_str,')/','classifiy_HCF_',p_regionType,'_total.mat');
            load(mat_path)
            total_rad_feat_num = 0;
            % fold 구분 없이 순서대로 저장되어 있어 classifier별 total radiomic feature개수 필요
%             if p_regionType == 'intra'
%                 total_rad_feat_num = 69-1;
%             elseif p_regionType == 'peri3mm' || p_regionType == 'peri12mm'
%                 total_rad_feat_num = 58-1;
%             elseif p_regionType == 'comb6mm' || p_regionType == 'comb9mm'
...
%             csv_path_rad_name = strcat(path_dirbase_csv,'fold(',date_str,')/',p_regionType,'_testfold(',fold,')_test.csv');
...
%         cln_rad_feat_weight = [cln_rad_feat_weight;div_fold];       % fold별 feature weight을 볼 수 있도록 결합
        ...
        result_byfeatnum = zeros(feature_number_total-1,7);     % feature 개수 별 결과 저장하는 변수 선언
       ...
       %% feature 2 to all loop 
        ...
        for num_of_selected_features = 2:feature_number_total
           ...
           %% classifier가 SVM 인 경우
            if p_clf(idx_fold) == "svm"
...
%                 'KernelScale','auto'); %, 'Prior', 'empirical'); %,'CategoricalPredictors', (1:3)
                ...
                test_label_svm=predict(mdl_svm,testset_selected_features);
                ...
                result_byfeatnum((num_of_selected_features-1),:) = [num_of_selected_features,temp_result_svm];
                
                clear 'temp' 'temp1' 'temp2'

                save_path_byfeatnum = strcat(path_save_fold, num2str(num_of_selected_features), '_features used/');
                save_byfeatnum = strcat(save_path_byfeatnum, p_regionType, '_testfold-', fold, '_svm.mat');

                if(isfolder(save_path_byfeatnum)==0)
                    mkdir(save_path_byfeatnum);
                end
                save(save_byfeatnum, 'test_label_svm', 'test_score_svm', 'temp_result_svm', 'trainset', 'train_gt', 'testset', 'test_gt',...
                    'trainset_selected_features','testset_selected_features');
            end 
        
           %% classifier가 random forest인 경우
            if p_clf(idx_fold) == "rf"
                rf_iter = 15;
                %result_rf_temp = zeros(1,6);
...
%                        'HyperparameterOptimizationOptions',struct('Showplots', false)); % 자동 하이퍼파라미터 최적화
               ...
               result_byfeatnum((num_of_selected_features-1),:) = [num_of_selected_features,result_rf];
               
               save_path_byfeatnum = strcat(path_save_fold, num2str(num_of_selected_features), '_features used/');
               save_byfeatnum = strcat(save_path_byfeatnum, p_regionType,'_testfold-', fold, '_rf.mat');
               ...
               save(save_byfeatnum, 'test_label_rf', 'test_score_rf', 'test_label_rf_all', 'test_score_rf_all',...
               'I', 'result_rf', 'result_rf_mean', 'trainset', 'train_gt', 'testset', 'test_gt',...
               'trainset_selected_features','testset_selected_features');
                clear 'I' 'test_label_rf_all' 'test_score_rf_all'
            end
            
        end
    fprintf(' > %s fold test end\n', fold);

    col = [loop_fold(idx_fold,1), p_clf(idx_fold,1),"","","","","";"featnum","ACC", "SEN", "SPEC", "PPV", "NPV","AUC"];
    cv_result = [cv_result; col; result_byfeatnum];
...
%     writematrix(cln_rad_feat_weight, save_path_whg);
    ...
    save_path = strcat(path_dirbase_csv, 'output(',date_str,')/', p_regionType,'_clnNrad_result',opt,'.csv'); %output
    ...
    clearvars -except 'date_str' 'dir_base_csv' 'loop_fold' 'num_of_rf_trees' 'path_save_base' 'opt' 'p_regionType' 'p_clf' 'varargin'
end