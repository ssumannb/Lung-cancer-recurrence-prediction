...
'''
df = pd.read_csv(".\csv\i_clinical_info_217.csv")
...
categorical = pd.DataFrame(df.iloc[:,0:11])                             # categorical variable
numerical = pd.DataFrame(df.iloc[:,11:])                                # numerical variable
...
'''
# 순서 있는 범주형 데이터 처리
categorical_4_order = categorical_3.iloc[:, 0:4]
...
categorical_4_wo_order = categorical_3.iloc[:, 4:]

double_val = ["pathology-location","anatomic_organ_subdivision"]

categorical_4_double = categorical_4_wo_order[:][double_val]

categorical_4_wo_order = categorical_4_wo_order.drop(["pathology-location", "anatomic_organ_subdivision"], axis=1)
